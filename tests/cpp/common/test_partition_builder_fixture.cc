...
class OptPartitionBuilderTestFixture : public testing::TestWithParam<std::tuple<size_t, size_t, int32_t>> {
...
public:
	...
	void CommonPartitionCheck(const GHistIndexMatrix& gmat, const ColumnMatrix& column_matrix, const RegTree& tree) {
	  switch (column_matrix.GetTypeSize()) {
		case common::kUint8BinsTypeSize:
          CommonPartitionCheckImpl<typename common::BinTypeMap<common::kUint8BinsTypeSize>::type>(gmat, column_matrix, tree);
          break;
        case common::kUint16BinsTypeSize:
          CommonPartitionCheckImpl<typename common::BinTypeMap<common::kUint16BinsTypeSize>::type>(gmat, column_matrix, tree);
          break;
        default:
          CommonPartitionCheckImpl<typename common::BinTypeMap<common::kUint32BinsTypeSize>::type>(gmat, column_matrix, tree);
      }
	}
...
private:
	...
	void CommonPartitionCheckImpl(const GHistIndexMatrix& gmat, const ColumnMatrix& column_matrix, const RegTree& tree) {
	  ...
	  std::unordered_map<uint32_t, uint64_t> split_ind;//(1, fid*((gmat.IsDense() ? row_count_ : 1)));
